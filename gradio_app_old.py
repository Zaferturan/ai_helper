...
app_state = {
    ...
    'access_token': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxIiwiZW1haWwiOiJ6YWZlcnR1cmFuQG5pbHVmZXIuYmVsLnRyIiwiZXhwIjoxNzU3MDk4ODAwfQ.UMcu_64Ei87g6eZ7oRzXOUKtuVzOpDgU5m64jWJ8ZEY',
    ...
    'is_admin': True,  # Admin kontrolü için
...
def send_login_code(email):
    """E-posta adresine giriş kodu gönder"""
    try:
        response = requests.post(f"{BACKEND_URL}/auth/send", json={"email": email})
...
def verify_login_code(email, code):
    ...
    try:
        response = requests.post(f"{BACKEND_URL}/auth/verify-code", json={"email": email, "code": code})
        if response.status_code == 200:
            data = response.json()
            ...
            return (
                ...
                gr.update(visible=True, value=user_info),  # user_info_html
                gr.update(visible=True),    # logout_btn
                gr.update(visible=True),    # main_app_area
                "✅ Giriş başarılı!"        # status message
            )
        else:
            return (
                gr.update(), gr.update(), gr.update(), gr.update(), gr.update(),
                gr.update(), gr.update(), gr.update(),
                f"❌ Geçersiz kod: {response.text}"
            )
    except Exception as e:
        return (
            gr.update(), gr.update(), gr.update(), gr.update(), gr.update(),
...
with gr.Blocks(
    ...
    with gr.Row():
        with gr.Column():
            # Email giriş ekranı
            login_title = gr.HTML("<h2>🔐 AI Helper - Giriş</h2>")
            login_subtitle = gr.HTML("<p>Bursa Nilüfer Belediyesi AI Yardımcı sistemine hoş geldiniz</p>")
            login_instruction = gr.HTML("<p>E-posta adresinizi girin, giriş için gerekli link ve kodu gönderelim</p>")
            ...
            # Kod doğrulama ekranı (başlangıçta gizli)
            code_title = gr.HTML("<h2>📧 Giriş için gerekli link ve kodu gönderdik</h2>", visible=False)
            code_subtitle = gr.HTML("<p>E-postandaki bağlantıya tıkla ya da aşağıya 6 haneli giriş kodunu yaz</p>", visible=False)
            ...
            with gr.Row(visible=False) as code_buttons:
                resend_btn = gr.Button("🔄 Tekrar Gönder (30)", variant="secondary")
                back_btn = gr.Button("⬅️ Geri Dön", variant="secondary")
            
            # Kullanıcı bilgileri (giriş sonrası görünür)
            user_info_html = gr.HTML(visible=False)
            logout_btn = gr.Button("🚪 Çıkış Yap", variant="secondary", visible=False)
    
    # Ana uygulama alanı (login sonrası görünür)
    with gr.Row(visible=False) as main_app_area:
    ...
    send_code_btn.click(
        ...
        outputs=[
            ...
            copy_result  # Status message için mevcut component kullan
        ]
    )
    ...
    verify_btn.click(
        ...
        outputs=[
            code_title, code_subtitle, code_input, verify_btn, code_buttons,
            user_info_html, logout_btn, main_app_area, copy_result
        ]
    )
    ...
    logout_btn.click(
        ...
        outputs=[
            login_title, login_subtitle, login_instruction, email_input, send_code_btn,
            code_title, code_subtitle, code_input, verify_btn, code_buttons,
            user_info_html, logout_btn, main_app_area, email_input,
            copy_result  # Status message için mevcut component kullan
        ]
    )
